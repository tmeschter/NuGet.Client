...
namespace NuGet.Packaging.Test
{
    ...
    {
        [Fact]
        public void GetPrimarySignatureSigningCertificates_WhenSignatureNull_Throws()
        {
            var exception = Assert.Throws<ArgumentNullException>(
                () => SignatureUtility.GetPrimarySignatureSigningCertificates(signature: null));

            Assert.Equal("signature", exception.ParamName);
        }

        [Fact]
        public void GetPrimarySignatureSigningCertificates_WithPrimarySignature_ReturnsCertificates()
        {
            var signature = Signature.Load(SignTestUtility.GetResourceBytes("SignatureWithTimestamp.p7s"));

            var certificates = SignatureUtility.GetPrimarySignatureSigningCertificates(signature);
        ...
        [Fact]
        public void GetPrimarySignatureTimestampSignatureSigningCertificates_WhenSignatureNull_Throws()
        {
            var exception = Assert.Throws<ArgumentNullException>(
                () => SignatureUtility.GetPrimarySignatureTimestampSignatureSigningCertificates(signature: null));

            Assert.Equal("signature", exception.ParamName);
        }

        [Fact]
        public void GetPrimarySignatureTimestampSignatureSigningCertificates_WithPrimarySignature_ReturnsCertificates()
        {
            var signature = Signature.Load(SignTestUtility.GetResourceBytes("SignatureWithTimestamp.p7s"));

            var certificates = SignatureUtility.GetPrimarySignatureTimestampSignatureSigningCertificates(signature);
